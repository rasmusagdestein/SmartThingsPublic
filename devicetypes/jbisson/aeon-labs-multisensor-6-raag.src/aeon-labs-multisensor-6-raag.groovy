 ...
 *  Revision History
 *  ==============================================
 *  2017-06-21 Version 1.2.1 Sync up and bunch of bug fixes
 *  2017-05-12 Version 1.2.0 Sync up and bunch of bug fixes
 *  2017-02-04 Version 1.1.0 Added timestamp on last battery received event
 *  2017-02-10 Version 1.0.0 Initial checkin
 *
 *
 *
 */
...
metadata {
    definition (name: "Aeon Labs MultiSensor 6 (RAAG)", namespace: "jbisson", author: "Eric Maycock (Modified by jbisson)") {
    ...
    preferences {
        input title: "", description: "Aeon MultiSensor 6 (gen5) v${clientVersion()}", displayDuringSetup: true, type: "paragraph", element: "paragraph"
    ...
    tiles (scale: 2) {
        ...
        standardTile("batteryTile", "device.batteryTile", inactiveLabel: false, decoration: "flat", width: 2, height: 2) {
            state "batteryTile", label:'${currentValue}', unit:"", action:"refreshBatteryLevel"
        }
        valueTile(
...
def parse(String description)
{
    ...
    switch(description){
        case ~/Err 106.*/:
            state.sec = 0
            result = internalCreateEvent([ name: "secureInclusion", value: "failed", isStateChange: true,
                    descriptionText: "This sensor failed to complete the network security key exchange. If you are unable to control it via SmartThings, you must remove it from your network and add it again."])
            break
        case "updated":
            result = internalCreateEvent([name: "Inclusion", value: "paired", isStateChange: true,
...
def zwaveEvent(physicalgraph.zwave.commands.securityv1.SecurityMessageEncapsulation cmd) {
    ...
    if (encapsulatedCommand) {
        ...
        internalCreateEvent([descriptionText: cmd.toString()])
    }
}
...
def zwaveEvent(physicalgraph.zwave.commands.batteryv1.BatteryReport cmd) {
    ...
    if(settings."101" == null || settings."101" == "241") {
        try {
            events << internalCreateEvent([name: "batteryTile", value: "Battery ${map.value}%\n ${new Date().format('yyyy-MM-dd \n HH:mm:ss',location.timeZone)}", displayed:false])
        } catch (e) {
            logging("$e")
        }
    }
    events << internalCreateEvent(map)
...
def zwaveEvent(physicalgraph.zwave.commands.sensormultilevelv5.SensorMultilevelReport cmd)
{
    ...
    if(request != []){
        return [response(commands(request)), internalCreateEvent(map)]
    } else {
        return internalCreateEvent(map)
    }
    ...
    if (cmd.notificationType == 7) {
        switch (cmd.event) {
            case 0:
                ...
                result << internalCreateEvent([name: "tamper", value: "clear", descriptionText: "$device.displayName tamper cleared"])
                result << internalCreateEvent([name: "acceleration", value: "inactive", descriptionText: "$device.displayName tamper cleared", displayed:false])
                break
            case 3:
                result << internalCreateEvent([name: "tamper", value: "detected", descriptionText: "$device.displayName was tampered"])
                result << internalCreateEvent([name: "acceleration", value: "active", descriptionText: "$device.displayName was moved", displayed:false])
                break
            case 7:
        ...
        result << internalCreateEvent([descriptionText: cmd.toString(), isStateChange: false])
    }
    result
}
...
def zwaveEvent(physicalgraph.zwave.commands.firmwareupdatemdv2.FirmwareMdReport cmd){
    ...
    internalCreateEvent([name: "currentFirmware", value: firmwareVersion])
}
...
private getBatteryRuntime(batteryRuntime) {
    def currentmillis = now() - batteryRuntime
    def days=0
    def hours=0
    def mins=0
    def secs=0
...
def refreshBatteryLevel() {
    logging("Refreshing battery level...")
...
private updateStatus(){
    ...
    if(state.batteryRuntimeStart != null){
        sendEvent(name:"batteryRuntime", value:getBatteryRuntime(state.batteryRuntimeStart), displayed:false)
        if (device.currentValue('currentFirmware') != null){
            sendEvent(name:"statusText2", value: "Firmware: v${device.currentValue('currentFirmware')} - Battery: ${getBatteryRuntime(state.batteryRuntimeStart)} Double tap to reset", displayed:false)
        } else {
            sendEvent(name:"statusText2", value: "Battery: ${getBatteryRuntime(state.batteryRuntimeStart)} Double tap to reset", displayed:false)
        }
    } else {
        state.batteryRuntimeStart = now()
    }
...
Default: No (Enable for Better Battery Life)
  ...
  <Value type="short" index="41" label="Temperature Threshold" min="1" max="5000" value="20" setting_type="zwave" fw="1.07,1.08,1.06EU,1.07EU">
    <Help>
...
Default: 5
    ...
    <Help>
The interval time of sending reports in Report group 1, This value affect the wakeUp interval for battery powered as well.
Range: 30~
...
Note: May help if config parameters aren't making it before device goes back to sleep.
  ...
  <Value type="boolean" index="forceStateChangeOnReport" label="Force state change when receiving a report ? If true, you'll always get notification even if report data doesn't change." 
    value="false" setting_type="preference" fw="1.06,1.07,1.08,1.06EU,1.07EU">
    <Help>
    </Help>
  </Value>
</configuration>
